...
- **Perubahan:**
  ...
  - **[CROSS-SUBDOMAIN BUGFIX]** Mengganti sistem penyimpanan persetujuan *Cookie Banner* dari yang awalnya menggunakan `localStorage` menjadi `document.cookie` dengan parameter `domain=.portotree.com`. `localStorage` tidak bisa membaca data lintas subdomain (isolasinya ketat), sehingga pengunjung akan ditanya berulang kali saat pindah ke halaman resume/portofolio. Dengan *HTTP Cookie*, sekali klik "Saya Mengerti", datanya otomatis berlaku universal untuk seluruh ekosistem `.portotree.com`.
