...
import { ChevronDown, User, ListChecks, FileText } from "lucide-react";
import { SignaturePad } from "@/components/ui/SignaturePad";

interface KesanggupanFormProps {
  formData: any;
  ...
  pernyataanList: any[];
  addPernyataan: () => void;
  updatePernyataan: (id: number, name: string) => void;
  removePernyataan: (id: number) => void;
}

export function KesanggupanForm({
  formData,
  handleFormChange,
  signatureData,
  setSignatureData,
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col p-4 gap-4">
        ...
        {expandedSection === 'data-diri' && (
          <div className="p-4 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all" placeholder="Contoh: Mahasiswa / Karyawan Swasta" value={formData.pekerjaan || ''} onChange={(e) => handleFormChange('pekerjaan', e.target.value)} />
            </div>
      ...
      {/* Section: Detail Kesanggupan */}
      <div className={`border rounded-lg overflow-hidden bg-white shadow-sm transition-colors duration-200 ${expandedSection === 'kesanggupan' ? 'border-emerald-200' : 'border-slate-200'}`}>
        <button 
          onClick={() => toggleSection('kesanggupan')}
          className={`w-full flex items-center justify-between p-4 transition-colors ${expandedSection === 'kesanggupan' ? 'bg-emerald-600' : 'bg-slate-50/50 hover:bg-slate-50 border-b border-slate-100'}`}
        >
          <div className="flex items-center gap-3">
            <ListChecks className={`w-4 h-4 ${expandedSection === 'kesanggupan' ? 'text-white' : 'text-emerald-500'}`} />
            <span className={`font-bold text-[15px] ${expandedSection === 'kesanggupan' ? 'text-white' : 'text-slate-700'}`}>Detail Kesanggupan</span>
          </div>
          <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${expandedSection === 'kesanggupan' ? 'text-white rotate-180' : 'text-slate-400'}`} />
        </button>
        {expandedSection === 'kesanggupan' && (
        ...
        {expandedSection === 'info-surat' && (
          <div className="p-4 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all" placeholder="Contoh: PT. Maju Mundur" value={formData.tujuanPernyataan || ''} onChange={(e) => handleFormChange('tujuanPernyataan', e.target.value)} />
              <p className="text-[11px] text-slate-500">Isi institusi/tempat dimana surat ini ditujukan.</p>
            </div>
