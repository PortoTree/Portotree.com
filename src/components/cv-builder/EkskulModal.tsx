...
export interface EkskulFormData {
  id?: string;
  title: string;
  issuer: string;
  year: string;
...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: EkskulFormData) => void;
  initialData?: EkskulFormData | null;
}

export function EkskulModal({ isOpen, onClose, onSave, initialData }: Props) {
  const [formData, setFormData] = useState<EkskulFormData>({
    title: '',
    issuer: '',
    year: ''
  });
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
          <h2 className="text-xl font-bold text-slate-800">
            {initialData ? 'Edit Ekskul' : 'Buat Ekskul Baru'}
          </h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto custom-scrollbar flex-1">
          <div className="space-y-5">
            <div className="space-y-2">
              <Label className="text-sm font-medium text-slate-700">Nama Kegiatan <span className="text-red-500">*</span></Label>
              <Input 
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium text-slate-700">Penyelenggara</Label>
              <Input 
